...
class ProjectSelector:
    ...
    def open_project_selection(self):
        if not self.game.project_in_progress:
            ...
            tree.heading(
                "name", text="Name", command=lambda: self.sort_projects(tree, "name")
            )
            tree.heading(
                "duration",
                text="Duration",
                command=lambda: self.sort_projects(tree, "duration", numeric=True),
            )
            tree.heading(
                "programming",
                text="Programming",
                command=lambda: self.sort_projects(tree, "programming", numeric=True),
            )
            tree.heading(
                "design",
                text="Design",
                command=lambda: self.sort_projects(tree, "design", numeric=True),
            )
            tree.heading(
                "marketing",
                text="Marketing",
                command=lambda: self.sort_projects(tree, "marketing", numeric=True),
            )
            ...
            def on_project_select(event):
                ...
                for project in self.game.projects:
                    if project.name == selected_project[0]:
                        self.game.project_manager.start_project()
