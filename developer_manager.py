...
class DeveloperManager:
    ...
    def open_hire_developer_selection(self):
        ...
        tk.Button(
            hire_window, text="Beginner Developer", command=hire_beginner_developer
        ).pack(pady=10)
        tk.Button(
            hire_window, text="Advanced Developer", command=hire_advanced_developer
        ).pack(pady=10)
        tk.Button(
            hire_window,
            text="Professional Developer",
            command=hire_professional_developer,
        ).pack(pady=10)

    def hire_developer(self, type):
        if type == "Beginner":
            new_developer = Developer.create_new_developer_beginner(
                self.game.developers
            )
        elif type == "Advanced":
            new_developer = Developer.create_new_developer_advanced(
                self.game.developers
            )
        elif type == "Professional":
            new_developer = Developer.create_new_developer_professional(
                self.game.developers
            )
